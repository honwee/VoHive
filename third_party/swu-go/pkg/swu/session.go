		...
		for {
			...
			if s.xfrmMgr != nil {
				if outsa := s.ChildSAOut; outsa != nil {
					if useTime, err := s.xfrmMgr.GetSALastUsed(outsa.SPI, s.xfrmLocalIP, s.xfrmRemoteIP, driver.XFRMProtoESP); err == nil && useTime > 0 {
...
func (s *Session) setupXFRMDataPlane() error {
	...
	outSACfg := driver.XFRMSAConfig{
		...
		Proto:         driver.XFRMProtoESP,
		Mode:          driver.XFRMModeTunnel,
		IsAEAD:        isAEAD,
		EncapType:     driver.XFRMEncapESPinUDP,
	...
	outSP4 := driver.XFRMSPConfig{
		Src:       allIPv4,
		Dst:       allIPv4,
		Dir:       driver.XFRMDirOut,
		TmplSrc:   localIP,
		TmplDst:   remoteIP,
		TmplProto: driver.XFRMProtoESP,
		TmplMode:  driver.XFRMModeTunnel,
	...
	outSP6 := driver.XFRMSPConfig{
		Src: allIPv6, Dst: allIPv6, Dir: driver.XFRMDirOut,
		TmplSrc: localIP, TmplDst: remoteIP,
		TmplProto: driver.XFRMProtoESP, TmplMode: driver.XFRMModeTunnel,
		TmplSPI: int(outSACfg.SPI), // 显式绑定 SPI
		Ifid:    int(xfrmIfID),
	}
	// Panic removed
	...
	inSP6 := driver.XFRMSPConfig{
		Src: allIPv6, Dst: allIPv6, Dir: driver.XFRMDirIn,
...
func (s *Session) rekeyXFRMSA(oldOutSPI, oldInSPI uint32, newSAOut, newSAIn *ipsec.SecurityAssociation, encrID uint16, encrKeyLenBits int) error {
	...
	// 1. 删除旧 SA
	_ = xfrmMgr.DelSA(oldOutSPI, localIP, remoteIP, driver.XFRMProtoESP)
	_ = xfrmMgr.DelSA(oldInSPI, remoteIP, localIP, driver.XFRMProtoESP)
	...
	for _, src := range []*net.IPNet{allIPv4, allIPv6} {
		_ = xfrmMgr.AddSP(driver.XFRMSPConfig{
			Src: src, Dst: src, Dir: driver.XFRMDirOut,
			TmplSrc: localIP, TmplDst: remoteIP,
			TmplProto: driver.XFRMProtoESP, TmplMode: driver.XFRMModeTunnel,
	...
	for _, src := range []*net.IPNet{allIPv4, allIPv6} {
		_ = xfrmMgr.AddSP(driver.XFRMSPConfig{
			Src: src, Dst: src, Dir: driver.XFRMDirIn,
			TmplSrc: remoteIP, TmplDst: localIP,
			TmplProto: driver.XFRMProtoESP, TmplMode: driver.XFRMModeTunnel,
