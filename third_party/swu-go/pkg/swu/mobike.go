...
func (s *Session) updateXFRMState(localAddr, remoteAddr string) error {
	...
	if s.ChildSAOut != nil {
		saCfg := driver.XFRMSAConfig{
			Src: lIP, Dst: rIP,
			SPI: s.ChildSAOut.SPI, Proto: driver.XFRMProtoESP,
			Mode: driver.XFRMModeTunnel, IsAEAD: isAEAD,
	...
	if s.ChildSAIn != nil {
		saCfg := driver.XFRMSAConfig{
			Src: rIP, Dst: lIP,
			SPI: s.ChildSAIn.SPI, Proto: driver.XFRMProtoESP,
			Mode: driver.XFRMModeTunnel, IsAEAD: isAEAD,
			EncapType:    driver.XFRMEncapESPinUDP,
